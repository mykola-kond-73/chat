<template>
  <div class="container" @mouseleave="isShowActions = false">
    <div
      ...
      :class="{ myMsg: isMyMsg, fremdMsg: !isMyMsg }"
      @mouseenter="isShowActions = true"
    >
      <div v-if="message.isUpdate" class="updated">
        Оновлено
      </div>
      <p class="message">
        {{ message.message }}
    ...
    <div
      ...
      class="actions"
      :class="{ myMsgAction: isMyMsg, fremdMsgAction: !isMyMsg }"
    >
      <div
        class="delete"
        @click="deleteMessage({ messageId: message.id, authorId: message.authorId,socket,roomId })"
      >
        x
      </div>
      <div
        v-if="userId == message.authorId"
        class="edit"
        @click="updateMessageHandler(message.message, message.id)"
      >
        e
      </div>
    </div>
